    ...
    def __init__(self, raw_data: np.ndarray, q_per_pixel: float):
        ...
        with st.sidebar.expander("📏 Kalibracja q/pixel", expanded=True):
            ...
            if self.use_manual_q:
                ...
                st.caption(
                    "Kalibracja z pliku jest już przeliczona do konwencji Q=2π/d "
                    "(patrz get_safe_scale). Ten suwak to tylko drobna korekta "
                    "(np. na niepewność środka wiązki), NIE zamiana konwencji."
                )
                self.q_scale_correction = st.slider(
                    "Mnożnik korekcyjny q/pixel (drobna korekta, ~1.0)",
                    min_value=0.90,
                    max_value=1.10,
                    value=1.000,
                    step=0.005,
...
def _convert_scale_to_A_inv_g(scale: float, units: str):
    """Konwertuje surową skalę osi (odczytaną z axes_manager) na jednostki
    Å⁻¹, w konwencji krystalograficznej g = 1/d (BEZ czynnika 2π).

    Pliki TIA/FEI (.emi/.ser) i większość formatów TEM (dm3/dm4 itp.)
    zapisują kalibrację dyfrakcji jako g = 1/d, zwykle w jednostkach 1/nm.
    Zwraca None, jeśli jednostki są nierozpoznane - wtedy trzeba zapytać
    użytkownika wprost.
    """
    if units is None:
        return None
    u = units.strip().lower().replace(" ", "")
    if u in ("1/nm", "nm^-1", "nm-1", "nm⁻¹"):
        return scale / 10.0
    if u in ("1/a", "1/å", "a^-1", "å^-1", "a-1", "å-1", "1/angstrom", "angstrom^-1"):
        return scale
    if u in ("1/m", "m^-1", "m-1"):
        return scale * 1e-10
    if u in ("1/pm", "pm^-1", "pm-1"):
        return scale * 100.0
    return None


def get_safe_scale(signal) -> float:
    """Zwraca kalibrację q/pixel w Å⁻¹/pixel, w KONWENCJI FIZYCZNEJ
    Q = 2*pi/d (czyli Q = 4*pi*sin(theta)/lambda), zgodnej z formułą
    transformaty G(r) używaną dalej w skrypcie (linia z (2/np.pi)*trapezoid(...)).

    Surowa kalibracja z pliku (axes_manager.scale) jest w konwencji
    krystalograficznej g = 1/d - stąd mnożenie przez 2*pi poniżej.
    To NIE jest dowolny "mnożnik korekcyjny" - to wymagana zamiana
    konwencji, bez niej Q byłoby ~6.28x za małe (a przy dodatkowym
    pominięciu jednostek nm->Å całość może się różnić nawet ~60x).
    """
    scale_raw = None
    units = None

    try:
        scale_raw = float(signal.axes_manager[0].scale)
        units = str(signal.axes_manager[0].units)
    except Exception:
        scale_raw = None

    g_per_px_A = None
    if scale_raw is not None and np.isfinite(scale_raw) and scale_raw > 0:
        g_per_px_A = _convert_scale_to_A_inv_g(scale_raw, units)

    if g_per_px_A is not None:
        q_per_px_A = g_per_px_A * 2.0 * np.pi  # g=1/d  ->  Q=2*pi/d
        st.sidebar.success(
            f"Kalibracja z pliku: {scale_raw:.6g} {units} (konwencja g=1/d) "
            f"→ **{q_per_px_A:.6f} Å⁻¹/pixel** (Q = 2π/d, po konwersji jednostek i ×2π)."
        )
        return float(q_per_px_A)

    if scale_raw is None or not np.isfinite(scale_raw) or scale_raw <= 0:
        st.sidebar.warning(
            "Nie udało się odczytać poprawnej skali osi. Podaj ręcznie q/pixel "
            "(w konwencji Q=2π/d, Å⁻¹/pixel)."
        )
    else:
        st.sidebar.warning(
            f"Jednostki osi z pliku ('{units}') nie zostały rozpoznane "
            f"(surowa wartość: {scale_raw:.6g}). Podaj ręcznie poprawną kalibrację "
            "Q=2π/d w Å⁻¹/pixel, albo przelicz ją samodzielnie: "
            "Q_per_px[Å⁻¹] = g_per_px[1/nm] * 2π / 10."
        )

    scale = st.sidebar.number_input(
        "Skala q/pixel (Å⁻¹/pixel, konwencja Q=2π/d)",
        min_value=1e-6,
        value=0.01,
        format="%.6f",
    )
