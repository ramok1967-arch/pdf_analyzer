...
class PDFWebAnalyzer:
    def __init__(self, raw_data, pixel_size):
        ...
        # Maska wiązki pierwotnej (beamstop)
        self.beamstop_radius = st.sidebar.slider("Promień maski wiązki (px)", 0, 100, 0, 1)
    ...
    def run(self):
        ...
        with col1:
            ...
            # Wizualizacja maski beamstop na podglądzie obrazu (zerowanie środka dla czytelności)
            display_img = self.image.copy()
            if self.beamstop_radius > 0:
                y, x = np.indices(display_img.shape)
                r_dist = np.sqrt((x - self.center[0])**2 + (y - self.center[1])**2)
                display_img[r_dist <= self.beamstop_radius] = 0
                
            ax1.imshow(np.log10(display_img + 1), cmap='magma')
...
if uploaded_files:
    # Tworzenie unikalnego folderu dla danej sesji przeglądarki
    if 'session_dir' not in st.session_state:
        st.session_state.session_dir = f"temp_analysis_{uuid.uuid4().hex[:8]}"
    ...
    # Bezpieczne czyszczenie zasobów po usunięciu plików z uloadera
    if 'session_dir' in st.session_state:
        temp_dir = st.session_state.session_dir
        if os.path.exists(temp_dir):
            shutil.rmtree(temp_dir, ignore_errors=True)
        del st.session_state['session_dir']
